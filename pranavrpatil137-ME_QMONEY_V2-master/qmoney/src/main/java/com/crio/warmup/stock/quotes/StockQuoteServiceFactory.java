...
public enum StockQuoteServiceFactory {
  ...
  public TiingoService getTiingoService(RestTemplate restTemplate){
       return new TiingoService(restTemplate);
  }

  public  AlphavantageService getAlphavantageService(RestTemplate restTemplate){
  ...
  public StockQuotesService getService(String provider,  RestTemplate restTemplate) {
    ...
    if(provider==null) return getAlphavantageService(restTemplate);
    if(provider.equalsIgnoreCase("tiingo")) {
        return getTiingoService(restTemplate);
     }else{
     return getAlphavantageService(restTemplate);
      }
     }
